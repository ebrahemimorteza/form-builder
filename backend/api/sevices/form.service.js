...
const saveClassData = (formJson) => {
	try {
		const url = uuidv4();
		const date = new Date().toLocaleString();
		const data = readFileSync(CLASS_DATA_FILE_URL);
		let dataJson = JSON.parse(data);
		dataJson.push({ url, formJson, date });
		writeFileSync(CLASS_DATA_FILE_URL, JSON.stringify(dataJson));
		return url;
	} catch (e) {
		console.error(e);
	}
};
const saveUserData = (formJson) => {
	console.log("saveUserData");
	try {
		const url = uuidv4();
		const date = new Date().toLocaleString();
		const data = readFileSync(USER_DATA_FILE_URL);
		let dataJson = JSON.parse(data);
		dataJson.push({ url, formJson, date });
		writeFileSync(USER_DATA_FILE_URL, JSON.stringify(dataJson));
		return url;
	} catch (e) {
		console.error(e);
	}
...
const getAllClasses = () => {	
	try {
		...
		return JSON.parse(readFileSync(CLASS_DATA_FILE_URL));
	} catch (e) {
		console.error(e);
	}
};
const getAllUsers = () => {	
	try {
		...
		return JSON.parse(readFileSync(USER_DATA_FILE_URL));
	} catch (e) {
		console.error(e);
	}
};
